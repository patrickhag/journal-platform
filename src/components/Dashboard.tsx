...
//type TAticle = z.infer<typeof articleSubmitionSchema> & {id: string, commentsForEditor: string}
export default async function DashboardPannel() {
 const articles = await db.select().from(articleSubmissions).leftJoin(files, eq(articleSubmissions.id, files.articleId))
  ...
  //? (
    ...
    //    }}
    //  >
    //    {session.data?.user.email} : Sign Out {articles.length}
    //  </Button>
    //) : (
 ...
 console.log(currentUser, articles)
      ...
      <main className="container py-8 mx-auto">
        <div className="flex flex-col gap-8">
          <div className="flex flex-col gap-1">
            <h2 className="text-2xl font-semibold">Welcome back, {currentUser?.email}</h2>
              ...
              <Card key={a.article_submissions.id}>
                <CardContent className="flex items-center justify-between p-6">
                  <div className="flex items-start gap-6">
                    <span className="text-xl font-medium text-muted-foreground">{(i+1).toString().padStart(4, '0')}</span>
                    <div className="space-y-1">
                      <h3 className="font-medium uppercase">{a.article_submissions.section}</h3>
                      <CommentPreview html={a.article_submissions.commentsForEditor} />
                    </div>
                  </div>
                  <DropdownMenu>
